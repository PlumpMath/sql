...
#ifdef SQL_DURATION_STORAGE
    return std::chrono::duration_cast<std::chrono::duration<Rep, Period>>(duration_storage(column.get<duration_storage::rep>()));
#else
    return date::parse_duration<Rep, Period>(column.get<std::string>());
#endif
  }
};
...
#else
    return date::parse_time_point<Clock, Duration>(column.get<std::string>());
#endif
  }
};
